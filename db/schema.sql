...
CREATE VIEW routers_and_device AS select *
from routers r
join devices d on r.device_id=d.id
/* routers_and_device(id,device_id,name,ssh_user,site,installed_at,active,password_id,fMain,parent_router_id,last_seen_at,"id:1",ros_version,model,serial_number,created_at,comment) */;
...
CREATE TABLE routers (id INTEGER PRIMARY KEY, device_id INTEGER, name TEXT NOT NULL, ssh_user TEXT NOT NULL, site TEXT, installed_at TEXT DEFAULT (date('now')) NOT NULL, active INTEGER DEFAULT 1, password_id INTEGER NOT NULL, fMain INTEGER NOT NULL DEFAULT 0, parent_router_id INTEGER REFERENCES routers (id), last_seen_at TEXT, FOREIGN KEY (password_id) REFERENCES passwords (id));
...
CREATE TABLE routers_addresses (router_id INTEGER, ip TEXT NOT NULL, comment TEXT, fMain INTEGER NOT NULL DEFAULT (0) CHECK (fMain IN (0, 1)), PRIMARY KEY (router_id, ip), FOREIGN KEY (router_id) REFERENCES routers (id));
